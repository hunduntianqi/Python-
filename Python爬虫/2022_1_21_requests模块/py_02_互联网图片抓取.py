...
class BaiDuPic:
    ...
    def dowaload(self, i, j, url, name, word):
        ...
        with open(
                r'D:\Users\Administrator\IdeaProjects\python_爬虫\2022_1_21_requests模块\{}\{}_{}_{}.jpg'.format(word, i, j,
                                                                                                             name),
                'wb') as file:
            file.write(pic_data)
    ...
    def run(self):
        ...
        try:
            os.mkdir(r'D:\Users\Administrator\IdeaProjects\python_爬虫\2022_1_21_requests模块\{}'.format(word))
        except:
            print('文件夹已存在！！')
        ...
        for i in range(0, num_pic):
            ...
            for j in range(len(list)):
                ...
                self.dowaload(i, j + 1, link[0].strip(),
                              ...
                              .replace('&', '_').replace('ǖ', '_').replace('\x08', '_').replace('\x01', '_').replace('\x02', '_')
                              .replace('\x03', '_').replace('\x04', '_').replace('\x05', '_').replace('\x06', '_').replace(
