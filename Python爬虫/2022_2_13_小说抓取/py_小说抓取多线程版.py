...
def bookSpider(book_type):
    print(book_type)
    # 解析书籍各分类详情链接
    for page in range(1, 11):
        book_type_link = 'https://www.txt909.com/html/{}/{}.html'.format(book_type.xpath('./a/@href')[0].split('/')[2], page)
        book_type_title = book_type.xpath('./a/@title')[0]
        print(book_type_title, book_type_link)
        # 为每种书籍分类建立文件夹
        try:
            os.mkdir('./小说下载2/{}'.format(book_type_title))
        except:
            print('文件夹已存在！！')
        # 对每种书籍第一页发送请求
        book_source = requests.get(book_type_link, headers=headers, timeout=3)
        # 实例化etree对象
        ...
        for book in book_list:
            ...
            book_name = book.xpath('./a/@title')[0].strip().replace(':', '_')
            print(book_name, book_link)
            # 为每一本书创建文件夹保存
            try:
                os.mkdir('./小说下载2/{}/{}'.format(book_type_title, book_name))
            except:
                print('文件夹已存在！！')
            # 向书籍详情页发送请求
            book_content = requests.get(book_link, headers=headers, timeout=3)
            # 实例化etree对象
            ...
            # file_json = open(r'D:\Users\Administrator\IdeaProjects\python_爬虫\2022_2_13_小说抓取\小说下载2\{}\{}\{}.json'.format(book_type_title, book_name, book_name), 'w', encoding='utf-8')
            # file_json.write(json.dumps(book_introduct))
            # file_json.close()
            with open('./小说下载2/{}/{}/{}.json'.format(book_type_title, book_name, book_name), 'w', encoding='utf-8') as file:
                json.dump(book_introduct, file, ensure_ascii=False)
            book_img_link = tree.xpath('//*[@id="downInfoArea"]/div[3]/span/img/@src')[0]
            # 请求获取图片内容
            img_source = requests.get(book_img_link, headers=headers, timeout=3)
            with open('./小说下载2/{}/{}/{}.jpg'.format(book_type_title, book_name, book_name), 'wb') as file:
                file.write(img_source.content)
            # 获取书籍下载页面链接
            book_down_link = tree.xpath('//*[@id="mainstory"]/ul[1]/li[1]/a/@href')[0]
            book_homepage_source = requests.get(book_down_link, headers=headers, timeout=3)
            # 实例化etree对象
            book_down_tree = etree.HTML(book_homepage_source.text)
            # 解析数据下载链接
            book_download_link = book_down_tree.xpath('//*[@id="info"]/p[6]/a/@href')[0]
            # 下载书籍
            book_download = requests.get(book_download_link, headers=headers, timeout=3)
            with open('./小说下载2/{}/{}/{}.txt'.format(book_type_title, book_name, book_name), 'wb') as file:
...
for book_type in book_type_list:
    t = Thread(target=bookSpider, kwargs={'book_type':book_type})
    t.start()
    # t.join()
    # print(book_type)
    # # 解析书籍各分类详情链接
