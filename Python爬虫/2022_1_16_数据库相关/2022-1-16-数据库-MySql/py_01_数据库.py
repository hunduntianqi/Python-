'''
    2022-1-16-数据库-MySql-本质就是文件-存储和管理数据的仓库,数据按照一定的格式存储,用户可以对数据库中的数据进行增加,修改,删除,查询等操作！！！
     数据库有数据表组成
     数据表用来存储数据
     存储一类相同信息的一列：字段
     存储人或事物的详细描述信息的一行:记录
     能够表示唯一一行记录的特殊字段：主键(特点：不能为空,不能重复)
    数据库的分类：
    1.关系型数据库-数据表和数据表之间有一定关联的数据库
     指采用了关系模型来组织数据的数据库,关系模型指的是二维表格模型,好比Excel中的表格,强调使用表格存储数据
     核心要素：数据行,数据列,数据表,2022-1-16-数据库-MySql(数据表的集合)
     常用关系型数据库：Oracle, Microsoft SQL Server, MySQL, SQLite
    2.非关系型数据库-数据表和数据表之间没有关联的数据库
...
RDBMS说明：
 ...
 3.关系型数据库服务端管理着不同的数据库,每个数据库会有一系列的数据文件,数据保存在数据文件中,每个数据库可以理解成一个文件夹！！！
"""

"""
SQL-结构化查询语言:
 用来操作RDBMS的语言,通过SQL可以操作Oracle, Microsoft SQL Server, MySQL, SQLite等关系型数据库
 作用：实现数据库客户端和服务端之间的通信,是通信的桥梁
 SQL语言分类：
 ...
 说明：
  1.对于程序员来讲，重点是数据的增、删、改、查,必须熟练编写DQL,DML，能够编写DDL完成数据库、表的操作,其他操作了解即可！！！
  2.SQL语言不区分大小写！！！
"""
"""
数据库数据类型和约束-为更加准确的存储数据,保证数据的正确有效：
数据类型：
