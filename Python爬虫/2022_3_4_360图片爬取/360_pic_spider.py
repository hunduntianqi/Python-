...
def pic_down(i, num, web, keyword, headers):
    '''下载图片'''
    pic_data = web.find_element_by_xpath(
...
try:
    ...
    print("文件夹已存在！！")
# 根据关键字查询图片
web.find_element_by_xpath('//*[@id="search_kw"]').send_keys(keyword, Keys.ENTER)
try:
    web.find_element_by_xpath('//*[@id="chacha_tab_propmt"]/span[2]').click()
except:
    pass
count = int(input('请输入您想要加载的图片页数:'))
# 点击获取古装类型图片页
web.find_element_by_xpath('//*[@id="tag"]/div/ul/li[2]/a').click()
# 定义变量存储图片遍历次数
num1 = 0
num2 = 0
num3 = 0
...
while flag:
    for i in range(1, 5):
        time.sleep(1)
        li_list = web.find_elements_by_xpath('//*[@id="searchlist"]/div[1]/ul[{}]/li'.format(i))
