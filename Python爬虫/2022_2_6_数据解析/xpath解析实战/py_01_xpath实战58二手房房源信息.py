...
if __name__ == '__main__':
    # 创建链接对象链接数据库
    conn = pymysql.connect(host='localhost', port=3306, user='root', password='13480194858gpt', database='python_test')
    # 创建游标对象
    cursor = conn.cursor()
    # 定义请求头
    headers = {
    ...
    for i in range(50):
        ...
        for fangyuan in fangyuan_list:
            title = fangyuan.xpath('./a/div[2]/div[1]/div[1]/h3/text()')[0].strip()
            price = fangyuan.xpath('./a/div[2]/div[2]/p[2]/text()')[0].strip()
            all_price = fangyuan.xpath('./a/div[2]/div[2]/p[1]/span[1]/text()')[0].strip() + '万'
